...
const SphereImage = ({ src, position, scale, onClick, loaded }: any) => {
...
const Gallery = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      // Immediately create the sphere positions
      const sphereRadius = 33;
      const goldenAngle = Math.PI * (3 - Math.sqrt(5));

      const generated = imports
        .map((src, i) => {
          const y = 1 - (i / (imports.length - 1)) * 2;
      ...
      setTimeout(() => setRotationStarted(true), 1000);
    };
  ...
  return (
    <div className="w-full h-screen relative">
      ...
      {!imagesLoaded && (
        <div className="absolute inset-0 z-10 flex items-center justify-center text-white bg-black/80">
          Loading...
        </div>
      )}
